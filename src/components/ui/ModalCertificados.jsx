...
export default function ModalCertificados({ isOpen, onClose, certificados, selectedCert, onSelectCert }) {
    ...
    return (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4 sm:p-6" onClick={onClose}>
            <div 
                className="relative z-10 bg-gradient-to-br from-gray-900 via-gray-900 to-gray-950 border border-purple-500/40 rounded-2xl shadow-2xl shadow-purple-500/40 w-full max-w-4xl max-h-[90vh] overflow-hidden flex flex-col"
                onClick={(e) => e.stopPropagation()}
            >
                {/* Header */}
                <div className="sticky top-0 bg-gradient-to-r from-purple-900/40 via-gray-900/40 to-cyan-900/40 border-b border-purple-500/40 px-6 sm:px-8 py-5 sm:py-6 flex items-center justify-between gap-4 backdrop-blur-sm">
                    <h3 className="text-xl sm:text-2xl lg:text-3xl font-bold bg-gradient-to-r from-purple-300 to-cyan-300 bg-clip-text text-transparent flex-1">
                ...
                {/* Filtros - Tags */}
                {!selectedCert && (
                    <div className="sticky top-20 bg-gradient-to-r from-gray-900/60 to-gray-900/60 border-b border-purple-500/20 px-6 sm:px-8 py-4 backdrop-blur-sm">
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                            {certificadosFiltrados.map((cert, idx) => (
                                <button
                                    ...
                                    className="group bg-gradient-to-br from-gray-800 to-gray-900 border border-purple-500/30 hover:border-purple-400/60 rounded-lg p-4 text-left transition-all hover:shadow-lg hover:shadow-purple-500/20"
                                >
                ...
                {/* Footer */}
                <div className="h-1 bg-gradient-to-r from-purple-500/50 via-cyan-500/30 to-purple-500/50 shrink-0"></div>
            </div>
        </div>
    );
}
