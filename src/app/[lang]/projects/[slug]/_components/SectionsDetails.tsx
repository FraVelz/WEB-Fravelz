    ...
    <div className="prose prose-lg dark:prose-invert max-w-none">
      {project.whatILearned?.[L]?.length ? (
        <section className="mb-12" id="project-what-i-learned-section">
          <h2 className="mb-6 text-3xl font-bold text-zinc-900 dark:text-zinc-100" data-i18n="projects_what_i_learned">
          ...
          <ul className="space-y-3" id="project-what-i-learned-list">
            {project.whatILearned[L]!.map((item) => (
              <li key={item} className="flex items-start gap-3 text-zinc-700 dark:text-zinc-300">
                <svg
                  className="mt-0.5 size-6 flex-shrink-0 text-cyan-500"
                  fill="none"
                  stroke="currentColor"
                  viewBox="0 0 24 24"
                >
      ...
      {project.extraInfo?.[L]?.length ? (
        <section className="mb-12" id="project-extra-info-section">
          <h2 className="mb-6 text-3xl font-bold text-zinc-900 dark:text-zinc-100" data-i18n="projects_extra_info">
            {t.projects_extra_info || "Información extra"}
          </h2>
          <ul
            className="list-disc space-y-3 pl-6 text-zinc-700 marker:text-cyan-500 dark:text-zinc-300 dark:marker:text-cyan-400"
          ...
          <div className="rounded-lg bg-zinc-100 p-6 dark:bg-zinc-800">
            <ul className="space-y-2" id="project-technical-details-list">
              {project.technicalDetails[L]!.map((detail) => (
                <li key={detail} className="flex items-start gap-2 text-zinc-700 dark:text-zinc-300">
