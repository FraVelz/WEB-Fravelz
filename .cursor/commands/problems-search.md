...
Considerar siempre el **impacto en producción** (usuarios, SEO, seguridad, build/CI, i18n, rendimiento, accesibilidad) y
la **probabilidad** de que el problema ocurra en la web desplegada.
...
- **Build y CI**: `.github/workflows/ci.yml`, scripts en `package.json`, lockfile, variables de entorno necesarias en
  build (p. ej. `NEXT_PUBLIC_SITE_URL`).
...
- Datos en `src/utils/data/` (proyectos, certificados): slugs, URLs, imágenes faltantes, idiomas `es` / `en` / `ru` /
  `zh` desalineados.
