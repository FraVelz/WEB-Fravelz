...
Cuando el usuario **actualiza un archivo JSON de localización** y la **versión en español** (`public/locals/es/`) es la
que ha editado a mano.
...
El **mismo nombre de archivo** debe existir en las cuatro carpetas para un mismo “módulo” de textos (por ejemplo
`common.json`, `hero.json`).

## Qué debe adjuntar el usuario

1. El comando **/update-json** (o el archivo `.cursor/commands/update-json.md`) **o** indicación explícita de que aplica
   esta instrucción.
...
4. **Preservar** convenciones del JSON válido (comillas, escapes, sin comas finales) y cualquier sustitución o
   placeholder que ya use el proyecto (por ejemplo `{{nombre}}` si existiera).

## Resultado esperado

Los cuatro JSON del mismo nombre actualizados y **estructuralmente equivalentes**, con traducciones coherentes con el
español.
